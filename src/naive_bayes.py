...
class NaiveBayes:
    ...
    def estimate_class_priors(self, labels: torch.Tensor) -> Dict[int, torch.Tensor]:
        ...
        labels = labels.to(torch.int64) #ESTO LO HE AÑADIDO CUANDO ME HA DADO ERROR, PERO NO ENTIENDO MUY BIEN
        n_samples = labels.shape[0]
        class_count = torch.bincount(labels)
        class_priors: Dict[int, torch.Tensor] = {}
        for i in range(len(class_count)):
            class_priors[i] = class_count[i]/n_samples
    ...
    def estimate_conditional_probabilities(
        ...
        for c in range(num_classes):
            class_examples = features[labels == c]
            word_counts = class_examples.sum(dim=0)
            total_words = word_counts.sum()

            class_word_counts[c] = (word_counts+delta) / (total_words + delta*n_words)
    ...
    def estimate_class_posteriors(
        ...
        for c in range(num_classes):
            ...
            log_likelihood = torch.sum(torch.log(self.conditional_probabilities[c])* feature) #no entiendo muy bien por qué tengo que multiplicar por feature
